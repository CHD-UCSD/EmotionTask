...
fixation=visual.TextStim(win=win, ori=0, name='text_intro',text=u'+',font=u'Arial',pos=[0, 0.23], height=0.09,wrapWidth=1.2,color=u'white', colorSpace=u'rgb', opacity=1,depth=0.0)
...
def save_and_quit(complete=False):
    ...
    if complete:
        wb.save('data/complete_data/'+ '%s_%s' %(expInfo['participant'], expInfo['date']+'.xls'))
        core.quit()
...
#         scale = '', low=1, high=4)# for logging, its useful to give names, esp when there are 2 on-screen
#     ScaleWarning=visual.TextStim(win, text="", pos = [0,-0.85], height=0.06, wrapWidth= 1, color=u'Red')
#     emotionLabel=visual.TextStim(win, text=str.capitalize(emotion) + ' (1=least, 4=most)', pos = [0,0.8], height = 0.1)

#     ImageL1 = visual.ImageStim(win, image=u'images/am1/am1_%s/am1_%s_60.bmp' %(emotion, emotion), pos=[leftward, 0.45], size=[0.5, 0.5])
#     ImageL2 = visual.ImageStim(win, image=u'images/am2/am2_%s/am2_%s_60.bmp' %(emotion, emotion), pos=[leftward, -0.40],size=[0.5, 0.5])
#     ImageR1= visual.ImageStim(win, image=u'images/af1/af1_%s/af1_%s_60.bmp' %(emotion, emotion), pos=[rightward, 0.45], size=[0.5, 0.5])
#     ImageR2 = visual.ImageStim(win, image=u'images/af4/af4_%s/af4_%s_60.bmp' %(emotion, emotion), pos=[rightward, -0.40],size=[0.5, 0.5])
